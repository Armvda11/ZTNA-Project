...
func (l *Listener) buildTLSConfig() (*tls.Config, error) {
	...
	cert, err := tls.LoadX509KeyPair(l.cfg.Server.TLS.CertFile, l.cfg.Server.TLS.KeyFile)
	if err != nil {
	...
	clientCACert, err := os.ReadFile(l.cfg.Server.TLS.ClientCAFile)
	if err != nil {
		return nil, fmt.Errorf("impossible de lire la CA client %s: %w", l.cfg.Server.TLS.ClientCAFile, err)
	}

	clientCAs := x509.NewCertPool()
	if !clientCAs.AppendCertsFromPEM(clientCACert) {
		return nil, fmt.Errorf("impossible de parser la CA client depuis %s", l.cfg.Server.TLS.ClientCAFile)
	}
