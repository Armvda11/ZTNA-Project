...
CONNECT_REQ="{\"protocol_version\":1,\"action\":\"connect\",\"resource\":{\"type\":\"${RESOURCE_TYPE}\",\"host\":\"${RESOURCE_HOST}\",\"port\":${RESOURCE_PORT}}}"
...
python3 - "${GW_HOST}" "${GW_PORT}" "${DEVICE_CRT}" "${DEVICE_KEY}" \
           "${RESOURCE_TYPE}" "${RESOURCE_HOST}" "${RESOURCE_PORT}" "${MODE}" << 'PYEOF'
import sys, ssl, socket, json, struct

gw_host, gw_port, cert, key, res_type, res_host, res_port, mode = sys.argv[1:]
gw_port = int(gw_port)
...
def recvall(sock, n):
  ...
  while len(buf) < n:
    ...
    if not chunk:
      raise EOFError("connexion fermée pendant la lecture")
    buf += chunk
  return buf
...
connect_req = json.dumps(connect_req_obj)
payload = connect_req.encode()
tls.sendall(struct.pack(">I", len(payload)) + payload)
print(f"[mTLS] ConnectRequest envoyé : {connect_req}")

# Lecture de la réponse framed (4 bytes longueur + JSON)
resp_len = struct.unpack(">I", recvall(tls, 4))[0]
resp = json.loads(recvall(tls, resp_len).decode())
print()
...
print(f"\n[AUTORISÉ] Tunnel TCP ouvert vers {res_type}:{res_host}:{res_port}")
